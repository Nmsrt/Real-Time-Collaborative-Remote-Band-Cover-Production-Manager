...
-- auth/login system, so RLS policies below grant the anon role open
-- read/write access; this is the same effective access control as the
-- previous unauthenticated Express API (anyone with the app URL could hit
-- any endpoint). Add real policies scoped to auth.uid() if login is added.
--
-- All mutations (create/save/delete) go through the functions at the bottom
-- of this file rather than direct table access from the client, so there is
-- one choke point to add ownership checks to later instead of several.
--
...
create policy "public access" on references_tracks for all using (true) with check (true);
create policy "public access" on roles for all using (true) with check (true);
create policy "public access" on stem_links for all using (true) with check (true);
create policy "public access" on video_links for all using (true) with check (true);
create policy "public access" on latest_mixes for all using (true) with check (true);
...
begin
  ...
  select
    ...
    coalesce(value->>'note', ''), coalesce(value->>'createdAt', to_char(now(), 'YYYY-MM-DD'))
...
-- Delete a single row from a known child table. table_name is checked
-- against an explicit allowlist before being interpolated (via format('%I'))
-- into dynamic SQL, so this can't be used to touch an arbitrary table.
